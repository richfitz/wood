...
\begin{abstract}
  ...
  between 45\% and 48\%.
  %
  This estimate is larger than 81\% of our expert estimates, most of
  which substantially understimated the proportion of the world's
  flora that was woody.  The results of our survey highlight that
  global datasets can show patterns at odds with conventional wisdom.
\end{abstract}

\newpage
\section{Introduction}

% RGF: Changed "of plants" to "in plants" here, but something still a
% bit odd about this sentence.
...
woody?
% RGF: Not happy with this reference placement -- try and get it more
% neatly into the text.  Could go into the "However, we have no
% idea..." paragraph, perhaps.
...
analysis of the data with a survey in which we asked our question to
the broader community of botanists and other biologists.
% 
...
% RGF: After the reorder, this makes even less sense.  The
% introduction finishes up with a big discussion about lack of
% consensus, which was needed to motivate the original question when
% the data came first.  So, I suggest this:
% 
% Trim down the discussion of the mention of the survey in the
% preceeding paragraph.  Perhaps: "We felt that such a basic question
% would be essentially known.  
...
%
%
% RGF: If we go with "largest database" we need to be clear about what
% the contrast is with.
% 
% RGF: Need to decide on strategy for the Zanne paper here.
% 
% RGF: Along with Dave's comments, we need to work out what we are
% providing in terms of raw data.  We probably can't redistribute the
% raw Plant List, but we can distribute our list of "from" and "to"
% names.  This should probably be the database version of "from", and
% the final analysis version of "to".  We probably need to talk about
% this soon.
...
to date (see ZANNE et al. (submitted) \& Dryad reference). The core of
the woodiness dataset came from Kew Rotal Botanic Gardens dataset on
growth form \citep{Kew}.  We reduced the original 103 categories to a
binary classification following a ``functional'' definition of
woodiness as a plant with a ``prominent, persistent above-ground
stem''.  In 57 of 103 cases, the Kew growth form categories provided
enough information to classify the species as either woody or
herbaceous according to our definition (see Table \ref{tab:kew} for
specific decisions).  In addition, several smaller datasets were added
according to the same categorization (see ZANNE et al. (submitted)).
Note that in addition to species producing true wood (i.e., consisting
of secondary xylem), this definition classifies, among other groups,
palms, tree ferns, and bamboo as woody.
...
% RGF: What is "Plant List tools" -- is there a reference?  I didn't
...
formal analysis we excluded these species.
%
...
added) and the exact version from this manuscript is available via
Dryad (reference to be added).


\section{Results and Discussion}

There was strikingly little consensus among researchers as to the
percentage of species that are woody.  We received 293 responses from
31 countries, which ranged from estimates of 1\% to 90\% with a mean
...
survey, there was no difference in estimates from researchers who
trained in tropical countries compared with their temperate colleagues
(results not shown).
...
46.6--47.8\%).  This estimate is larger than 81\% of our survey
estimates.
...
than 90\% woody species), with a mean of 202 species rather than 138.
...
% RGF: Andrea suggested something about the fact that many forests are
% fairly species poor, outside of the hyperdiverse tropical forests.
% Alternatively, it could be that people think of trees, when in fact
% most shrubs are woody.
%
