...
\author{
Richard G. FitzJohn$^{*,1,2}$, Matthew W. Pennell$^{*,\dag,3,4}$, Amy E. Zanne$^{5,6}$,\\ Peter F. Stevens$^{7,8}$, David C. Tank$^{3,9}$, William K. Cornwell$^{10}$
}
\date{}
...
\item{
  ...
  surprising that the answer is not known.   The main challenge to answering this question and in all scaling of functional datasets to the global scale is dataset bias.  
  We currently have data on XX\% of taxonomically resolved species across the globe.  However, these data are not a random sample of global diversity: 
  some clades are over-sampled, while others have little to no functional data.  This sampling bias means that the global mean value may be very different from a simple dataset average.
  }
  % 
\item{
 Starting with a database of woodiness for 49,064 species of vascular plants (XX\% woody)
  we estimated the status of the remaining 235,668 taxonomically resolved species using two
...
\item{
 After accounting for sampling bias, we estimated the proportion of woodiness
  among the world's vascular plants to be between 45\% and 48\%.  This was much lower than the raw dataset average and much higher than the mean of the survey.  
  Our study highlights that that global datasets can show patterns at
  odds with both simple dataset means and with conventional wisdom.
}
  %
\item{
  \emph{Synthesis}: Alongside an understanding of global taxonomic diversity (i.e. number of species globally), building a functional understanding of global diversity is an important new research direction.  This approach represents a new way to use functional trait datasets to answer basic questions at a global scale.
}
\end{enumerate}
\end{abstract}
...
Functional trait datasets are almost without exception biased.  Researchers collect data for specific questions on a local scale, and
assembling these local datasets is a very useful step (TRY REF), but as with GenBank's assembly of genetic data (SMITH REF),
the simple compilation of data is not a unbiased sample of global diversity.  Understanding and accounting for the biases in these datasets is a 
important and necessary next step.  
......%NEED NEW TRANSITION HERE

We aim to use  re-ask Theophrastus' 2000 year old
...
classification following a ``functional'' definition of woodiness %ADD ASA GREY REFERENCE HERE
as a
...
%WKC: I know this now doesn't mirror the order of the methods, but I think it could be okay
...
\citep[e.g. TRY;][]{kattge2011try} are undoubtedly extremely useful in
