...
\item The question posed by the title of this paper is a basic one,
  ...
  scaling these datasets to the global scale is challening because of
...
woody.
%
%WKC be nice to stop using a completely unusable definition for "true wood", even if it's what's in textbooks currently
...
(\textit{https://github.com/richfitz/wood}) and are included
...
47.0--48.2\%) (Figure \ref{fig:distribution-raw}).
%
...
% WKC: "For example, for woodiness..." is awkward 
% What about: "For this woodiness dataset,..."
This sampling bias also affect many commonly used methods in
...
% I think there is a point we are missing here, which is the
% connection between our method and the "gap-filling" problem.  Maybe
% lead this paragraph by making that point.  I think that would be
% stronger than what we currently have.
%
% something like "The question of accounting for bias in generating
% global summary statistics is directly connected to the problem of
% phylogenetically-informed gap-filling.  In addition to improving an
% estimate of the mean, the methods in this paper can also be used to
% generate a probability of each unobserved species being woody.  Two
% related approaches have recently been developed to address this
% problem focusing on continuous traits..."

Recently, two related approaches have been developed to address the
problem of missing data in trait databases, both focusing on
continuous traits \citep{Swenson2013, PEM}.  While their details
differ, both approaches are model--based in that they impute trait
values for missing species based on the fitted parameters of
phylogenetic models estimated from the species already in the
database. This is conceptually different from our approach; we do not
assume any model for the evolution of woodiness, such as the `Mk'
model \citep{Pagel1994}, which is commonly used to model discrete
characters evolving on a phylogeny. Both types of approaches --- using
taxonomic categories (this study) versus modeling trait evolution
along a phylogeny --- have advantages and disadvantages.  One
disadvantage of a modeling--based approach is that if the sampling is
biased with respect to the character states, the parameter estimates
themselves will be biased, leading to incorrect estimation of the
states for the remaining species. While our approach avoids this
issue, we ignore potentially useful information on the phylogenetic
relationships within genera and branch lengths separating lineages.

\section{Concluding remarks}

As a result of centuries of effort, we now have a solid understanding 
of the taxonomic diversity of many groups and regions. By drawing on
huge amounts of accumulated research, the recent development of global trait
databases offers the promise of gaining similar insights into the functional 
diversity of the earth's biota.
While the question we ask in this paper --- what proportion of the world's 
flora is woody? --- is rather simple, answering it required dealing with the
diverse biases that are inevitably present in large datasets. Our approach is 
just one of many potential ways to address these biases; we hope that our analysis
encourages other researchers to think critically and creatively about the problem.
Making robust inferences about broad--scale ecological and evolutionary questions
from emerging global datasets will require tackling this issue head on. 
...
Dryad (\textit{to be added}) and on github \texttt{https://github.com/richfitz/wood}.
\end{itemize}
