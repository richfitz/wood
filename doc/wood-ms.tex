...
Our estimates of the percentage of species that are woody (45/47\%)
differ from the raw estimate based on species in our database (58\%).
...
compared to 137 (See Figure \ref{fig:variability}).
% #rstats
% dat.10 <- dat.g[dat.g$K > 10,]
% round(mean(dat.10$N[dat.10$p < .1])) # 202
% round(mean(dat.10$N[dat.10$p > .9])) # 137
...
This sampling 
bias also affect many commonly used methods in ecological and
...
% I think there is a point we are missing here, which is the connection between our method
% and the "gap-filling" problem.  Maybe lead this paragraph by making that point.  
% I think that would be stronger than what we currently have.
%
%something like
% The question of accounting for bias in generating global summary statistics is directly connected to the 
% problem of phylogenetically-informed gap-filling.  In addition to improving an estimate of the mean, 
% the methods in this paper can also be used to generate 
% a probability of each unobserved species being woody.  
%Two related approaches have recently been developed to address this problem focusing on continuous traits...
...
The development of global databases for functional traits
has been one of the recent key advances in ecology and evolutionary
research, bringing our functional understanding of the world's biota closer to our 
taxonomic understanding of it.  These databases offer the promise of allowing researchers to
