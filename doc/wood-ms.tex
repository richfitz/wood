...
For both the hypergeometric and binomial models, we drew 1000 samples 
of $N_w$ per genus. We then combined the samples from all genera to 
...
we do not assume any model for the evolution of woodiness \citep[such as the `Mk' model;][]{Pagel1994, Lewis2001}.
Methods which rely on a specific model
of evolution are susceptible to the problem that if the sampling is biased with respect
...
was the imputus for developing our approach, it seems highly inappropriate to use a 
method that assumes random sampling of trait values!
